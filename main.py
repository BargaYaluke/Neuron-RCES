...
def neuron_mrc_and_prune(args, model,
    ...
    for batch_idx, (inputs, targets) in enumerate(adv_loader):
        ...
        if use_contrastive:
            ...
            with torch.no_grad():
                _, feat2 = _extract_logits_and_feats(model, v2)
            ce = criterion(logits1, targets)
            ...
            logits1, _ = _extract_logits_and_feats(model, inputs)
            ce = criterion(logits1, targets)
            loss = (lambda_ce * ce) / num_grad_batches
